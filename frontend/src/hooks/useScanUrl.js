...
export const useScanUrl = (onScanComplete) => {
  ...
  const handleScan = useCallback(async () => {
    ...
    try {
      ...
      const scanData = response.data.data.scanUrl;
      setResults(scanData);
      ...
      if (
        scanData?.externalReports?.urlscan?.status === 'pending' &&
        scanData?.externalReports?.urlscan?.scanId
      ) {
        // Start polling for URLScan results
        setScanId(scanData.externalReports.urlscan.scanId);
