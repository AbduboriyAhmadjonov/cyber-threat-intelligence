  ...
  if (!data) {
    return (
      <div className={`p-4 ${darkMode ? 'bg-gray-800' : 'bg-gray-50'} rounded-b-lg`}>
        <p className="text-center italic text-gray-500">No VirusTotal data available</p>
      </div>
    );
  }
    ...
    <div className={`p-4 ${darkMode ? 'bg-gray-800' : 'bg-gray-50'} rounded-b-lg`}>
      <div className="flex items-center mb-4">
        ...
        <h3 className="font-medium">
          Status: {data.positives === 0 ? 'Clean' : `${data.positives} threats detected`}
        </h3>
      </div>
      <p>
        Detections: {data.positives || 0}/{data.total || 0} engines
      </p>
      <p>Scan Date: {data.scanDate ? new Date(data.scanDate).toLocaleString() : 'N/A'}</p>
      {data.message && (
        <p className="mt-2 italic text-gray-500 dark:text-gray-400">{data.message}</p>
      )}
    </div>
  );
};
