...
async function checkUrlSafety(url) {
  ...
  if (externalReports.googleSafeBrowsing && !externalReports.googleSafeBrowsing.safe) {
    isSafe = false;
  }
...
async function checkGoogleSafeBrowsing(url, externalReports) {
  try {
    const response = await googleSafeBrowsingClient.post('/threatMatches:find', {
      client: {
...
async function main() {
  ...
  try {
    ...
    console.log('Initial Security Check Results:', JSON.stringify(initialResult, null, 2));

    // Extract the scan ID from the initial result
    if (initialResult.externalReports.urlscan && initialResult.externalReports.urlscan.scanId) {
      ...
      console.log('urlscan.io Results:', JSON.stringify(urlscanResults, null, 2));
      ...
      console.log('\nFinal Security Check Results:', JSON.stringify(initialResult, null, 2));
    }
  } catch (error) {
...
async function checkUrlSafetyWithUrlscan(url, waitForUrlscan = true, waitTimeMs = 20000) {
  // Get initial results
  const result = await checkUrlSafety(url);

  // If we have a urlscan ID and want to wait for results
  if (waitForUrlscan && result.externalReports.urlscan && result.externalReports.urlscan.scanId) {
    // Wait specified time
    await new Promise((resolve) => setTimeout(resolve, waitTimeMs));

    // Get urlscan results
    const urlscanResults = await getUrlscanResults(result.externalReports.urlscan.scanId);
...
async function checkUrlscanStatus(scanId) {
  ...
  // the actual API call to URLScan.io to check the status

  // URLScan.io API endpoint to check scan status
  const response = await axios.get(`https://urlscan.io/api/v1/result/${scanId}`);

  console.log('Raw URLScan response:', JSON.stringify(response.data, null, 2));
  if (response.data && response.data.verdicts) {
    ...
    return {
      ...
      scanDate: response.data.task.time,
    };
  }

  // If the scan is still processing
  return {
    status: 'pending',
    scanId: scanId,
